...
def popular_articles():
    global all_articles
    popular_article_data = assign_val()
    popular_articles.append(popular_article_data)
    all_articles.drop([0], inplace=True)
    all_articles=all_articles.reset_index(drop=True)
...
def recommended_articles():
   ...
   column_names = ["url", "title", "text", "lang", "total_events"]
   all_recommended = pd.DataFrame(columns=column_names)
   
   for like_article in liked_articles:
        output = get_recommendations(like_article["title"])
